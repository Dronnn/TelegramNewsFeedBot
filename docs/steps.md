...
- Никаких секретов в коде или git. Только `.env.example` с плейсхолдерами
...
- [x] **Шаг 002**: Создать `.gitignore` (venv, .env, __pycache__, data/bot.db, logs/, *.session)
- [x] **Шаг 003**: Создать `.env.example` с плейсхолдерами (без реальных данных)
